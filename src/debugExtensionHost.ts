...
export class DebugExtensionHostFeature {
	constructor(context: ExtensionContext) {
		const data = context.globalState.get("attachToProcessId") as
			| Data
			| undefined;
		if (data) {
			(async () => {
				vscode.window.withProgress(
					{
						...
						title: "Attaching to Extension Host",
					},
					async () => {
		...
		commands.registerCommand(
			"vscode-diagnostic-tools.debug-extension-host",
			async () => {
				const result = (await commands.executeCommand(
					"workbench.extensions.action.debugExtensionHost",
					{ dryRun: true },
				)) as { inspectPorts: { port: number; host: string }[] };

				if (!result) {
					vscode.window.showErrorMessage(
						"Failed to get inspect ports.",
					);
					return;
				}
