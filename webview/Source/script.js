...
require(["vs/editor/editor.main"], function () {
	...
	const {
		...
		LineRangeMapping,
	} = require("vs/editor/common/diff/rangeMapping");
	...
	window.addEventListener("message", (e) => {
		...
		moves = message.moves.map(
			(m) =>
				new MovedText(
					new LineRangeMapping(
						parseLineRange(m.originalRange),
						parseLineRange(m.modifiedRange),
					),
		...
		const allInnerChanges = changes.flatMap(
			(c) =>
				c.innerChanges ||
				new RangeMapping(
					lineRangeToRange(c.original),
					lineRangeToRange(c.modified),
				),
		);
		let lcsLen = 0;
		...
		for (let i = 0; i <= allInnerChanges.length; i++) {
			...
			const startPosition = previousInnerChange
				? previousInnerChange.originalRange.getEndPosition()
				: new m.Position(1, 1);
			const endPosition = innerChange
				? innerChange.originalRange.getStartPosition()
				: new m.Position(originalModel.getLineCount() + 1, 1);
